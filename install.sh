...
if [ -z "$VERSION" ]; then echo "BVM Error: No version selected." >&2; exit 1; fi
[[ "$VERSION" != v* ]] && VERSION="v$VERSION"
VERSION_DIR="$BVM_DIR/versions/$VERSION"
if [ ! -d "$VERSION_DIR" ]; then echo "BVM Error: Not installed." >&2; exit 1; fi
export BUN_INSTALL="$VERSION_DIR"
export PATH="$VERSION_DIR/bin:$PATH"
exec "$VERSION_DIR/bin/$COMMAND" "$@"
EOF
