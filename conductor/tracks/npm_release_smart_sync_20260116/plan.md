...
## Phase 1: 基础配置与指纹
- [ ] Task: 更新 `package.json` 的 `files` 列表，包含所有安装资产。
- [ ] Task: 确保 `scripts/fingerprint.ts` 能够处理所有发布资产。
- [ ] Task: Conductor - 用户手动验证 'Phase 1' (Protocol in workflow.md)

## Phase 2: 智能安装脚本 (Unix/Windows)
- [ ] Task: 重构 `install.sh` 实现**动态版本解析** (从 NPM 查询最新版)。
- [ ] Task: 在 `install.sh` 中实现**智能网络探测** (自动切换 unpkg/elemecdn)。
- [ ] Task: 在 `install.ps1` 中同步上述逻辑。
- [ ] Task: 本地模拟 CN/Global 环境验证脚本的自适应能力。
